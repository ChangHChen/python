# ついにこんなに長いプログラムになってしまった、詳しくnoteを書きたかったが、試験期間で、復習に時間がかかるため、
# できなかった、申し訳ございません。
#大変であったが、ほんとにいい経験になったと思う。
import random
...
colors = ["r", "b", "d", "c"]
status = ["ready", "stand", "Bust", "Blackjack", "Surrendered"]
triggers = ["lost", "won", "got a Blackjack", "got a drew", "surrendered"]
...
class Player(object):
    ...
    def deal(self, decks):
        if len(decks) == 0:
            print("we've ran out of cards, 6 new decks have been shuffled")
            decks = set_up_decks(6)
            decks = shuffle(current_cards)
        self.hand.append(decks[0])
        del (decks[0])
...
print("6 decks of cards have been shuffled")
...
while True:
    print(f"** this is game <{round_count}>, there are {len(current_cards)} cards remains **")
    round_count += 1
    players_have_money = []
    ...
    if len(players_have_money) > 0:
        ...
        for i in range(2):
            computer.deal(current_cards)
            for everyone in players_have_money:
                everyone.deal(current_cards)
        for who_bet in players_have_money:
            print(computer.name, ": [Facing Down]", computer.hand[1])
            who_bet.check()
            who_bet.show()
            if who_bet.money >= who_bet.bets:
                y_or_n = input(f"would <{who_bet.name}> like to double down? y/n\n").lower()
                if y_or_n == "y":
                    who_bet.status = 5
            if who_bet.status == 3:
            ...
            while who_bet.status != 3:
                if who_bet.status == 2:
                    ...
                    break
                elif who_bet.status == 5:
                    who_bet.bet(who_bet.bets)
                    who_bet.deal(current_cards)
                    who_bet.show()
                    who_bet.check()
                    if who_bet.status != 2:
                        who_bet.status = 1
                elif who_bet.status == 4:
                    who_bet.win_or_lose(4)
                    break
                elif who_bet.status == 1:
                    break
                else:
                    while who_bet.status == 0:
                        command = input(f"what would <{who_bet.name}> do?('h' for hit, 's' for stand, 'sur' for surrender)\n")
                        if command == "h":
                            who_bet.deal(current_cards)
                            who_bet.show()
                            who_bet.check()
